...
export class PacketQueue extends Plugin {
    ...
    start(): void {
        ...
        try {
            this.socketManager = (document as any).highlite.gameHooks.SocketManager.Instance;
            ...
            this.originalEmitPacket = this.socketManager.emitPacket.bind(this.socketManager);
            ...
            this.log(`Started - Packet queuing active (${this.PACKET_INTERVAL_MS}ms intervals)`);
        } catch (error) {
    ...
    private queuePacket(packet: any): void {
        ...
        if (this.packetQueue.length === 0 && timeSinceLastSent >= this.PACKET_INTERVAL_MS) {
        ...
        if (packetName === 10 || packetName === 43) {
            ...
            this.packetQueue = this.packetQueue.filter(queuedPacket => {
                const queuedPacketName = queuedPacket._name || queuedPacket.name;
    ...
    private processQueue(): void {
        if (this.isProcessing || this.packetQueue.length === 0 || !this.originalEmitPacket || !this.isLoggedIn) {
            return;
        }
    ...
    getQueueStatus(): { size: number; isProcessing: boolean; isLoggedIn: boolean; lastSentTime: number; timeSinceLastSent: number; interval: number } {
        return {
